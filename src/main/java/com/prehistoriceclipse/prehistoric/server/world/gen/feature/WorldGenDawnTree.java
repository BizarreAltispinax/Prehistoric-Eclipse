package com.prehistoriceclipse.prehistoric.server.world.gen.feature;
...
public class WorldGenDawnTree extends WorldGenAbstractTree {
    ...
    public WorldGenDawnTree(boolean p_i2027_1_)
    {
        this(p_i2027_1_, 4, DEFAULT_TRUNK, DEFAULT_LEAF, false);
    }

    public WorldGenDawnTree(boolean notify, int minTreeHeightIn, IBlockState woodMeta, IBlockState p_i46446_4_, boolean growVines)
    {
        super(notify);
                            ...
                            for (int l4 = position.getX() - k4; l4 <= position.getX() + k4; ++l4)
                            {
                                for (int i5 = position.getZ() - k4; i5 <= position.getZ() + k4; ++i5)
                                {
                                    ...
                                    if (state.getBlock().isLeaves(state, worldIn, blockpos$mutableblockpos1))
                                    {
                                        ...
                                        if (rand.nextInt(20) == 0 && worldIn.isAirBlock(blockpos2))
                                        {
                                            this.addHangingVine(worldIn, blockpos2, BlockVine.EAST);
                                        }

                                        if (rand.nextInt(20) == 0 && worldIn.isAirBlock(blockpos3))
                                        {
                                            this.addHangingVine(worldIn, blockpos3, BlockVine.WEST);
                                        }

                                        if (rand.nextInt(20) == 0 && worldIn.isAirBlock(blockpos4))
                                        {
                                            this.addHangingVine(worldIn, blockpos4, BlockVine.SOUTH);
                                        }

                                        if (rand.nextInt(20) == 0 && worldIn.isAirBlock(blockpos1))
                                        {
    ...
    {
        this.setBlockAndNotifyAdequately(worldIn, pos, PEBlocks.GRAPE_VINES.getDefaultState().withProperty(prop, Boolean.valueOf(true)));
    }
